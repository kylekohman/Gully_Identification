...
class CustomDataset(utils.Dataset):
    ...
    def load_custom(self, dataset_dir, subset):
        ...
        if subset == "train":
            annotations = json.load(open("train80.json"))
        else:
            annotations = json.load(open("val20.json"))
        #annotations = json.load(open#(os.path.join(dataset_dir, "labels/" + subset + ".json")))
...
dataset_train.load_custom("gully_images_rgb_80-20_split/", "train")
dataset_train.prepare()
print('Train: %d' % len(dataset_train.image_ids))

dataset_val = CustomDataset()
dataset_val.load_custom("gully_images_rgb_80-20_split/", "val")
dataset_val.prepare()
...
class GullyConfig(Config):
    # define the name of the configuration
    NAME = "gully_cfg"
    # number of classes (background + gully)
    NUM_CLASSES = 1 + 1
...
model.train(dataset_train, dataset_train, learning_rate=config.LEARNING_RATE, epochs=100, layers='all', augmentation=augmentation)
'''
############################
# INFERENCE
...
# load model weights
model.keras_model.load_weights('mask_rcnn_gully_cfg_0100_aug.h5', by_name=True)
...
gully_img = skimage.io.imread("gully_images_rgb_all_train/test/607_36.png")
#plt.imshow(marbles_img)

detected = model.detect([gully_img])
results = detected[0]
class_names = ['BG', 'Gully']
display_instances(gully_img, results['rois'], results['masks'],
                  results['class_ids'], class_names, results['scores'])
#display_instances(image=gully_img, boxes=results['rois'], masks=results['masks'], class_ids=results['class_ids'], class_names, scores=results['scores'])
...
#detect_and_color_splash(model, image_path="gully_images/val/607_36.png")
